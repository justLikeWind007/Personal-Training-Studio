...
echo "[1/8] 启动中间件容器 (mysql/redis/rocketmq/es/nacos/sentinel)"
docker compose up -d mysql redis rocketmq-namesrv rocketmq-broker rocketmq-dashboard elasticsearch kibana nacos sentinel-dashboard >/dev/null
...
echo "[5/8] 等待 Elasticsearch 健康检查"
for i in {1..50}; do
...
echo "[7/8] 等待 Sentinel Dashboard 端口"
for i in {1..40}; do
  if bash -c "exec 3<>/dev/tcp/127.0.0.1/8718" 2>/dev/null; then
    break
  fi
  sleep 2
done
if ! bash -c "exec 3<>/dev/tcp/127.0.0.1/8718" 2>/dev/null; then
  echo "Sentinel Dashboard 未就绪"
  exit 1
fi
...
echo "Sentinel Dashboard:  http://127.0.0.1:8718"
