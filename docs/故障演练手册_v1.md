...
## 6. RocketMQ NameServer 故障演练

前置条件：
- 已启动 `docker-compose.cluster.yml` 中的 RocketMQ NameServer 与 Broker
- `docker` 可用
...
检查点：
- 停止一个 NameServer 后，另一个 NameServer 与 Broker 端口仍可达
- 被停止节点端口不可达，且恢复后重新可达

## 7. 后续建议

- 增加 RocketMQ 生产/消费消息级演练
