...
## v2.0 总目标
- [x] 多门店总部治理能力上线
- [x] 数据权限模型上线（`TENANT_ALL` / `STORE_ASSIGNED` / `SELF_ONLY`）
- [x] 按钮级权限配置页面上线
- [x] 财务一致性稽核中心上线
- [x] outbox 事件重试与补偿能力完善
- [x] 总部经营 BI 看板上线
...
### US-003 角色与按钮级权限配置页面
- [x] 角色管理页（角色、菜单、按钮）（已提供 `/admin/rbac.html` 调用后端接口联调）
- [x] 用户授权页（用户-角色绑定）（已在 `/admin/rbac.html` 集成）
...
- [x] FR-1 多门店组织管理
- [x] FR-2 总部/门店两级角色体系（新增角色分级目录接口 `/api/rbac/roles/catalog`）
- [x] FR-3 数据范围自动生效
- [x] FR-4 权限可视化配置
...
- [x] FR-10 安全回归测试入 CI 分层（`smoke/security/regression` 已完成分层并可执行）

## 验收里程碑
- [x] M2-1：多门店与数据权限模型
- [x] M2-2：权限配置页面 + 权限回归
- [x] M2-3：稽核中心 + outbox补偿
- [x] M2-4：总部BI + v2.0 验收
